...
function FastDownload(url, options, cb) {
    ...
    request.head(self._url, _.clone(self._options, true), function (error, response) {
        ...
        if (response.statusCode != 200) {
            console.log("Error in head request..");
            if (response.statusCode == 403) { //amazon.s3 forbids head requests
                
                if((self._options.file_size || 0) > 0) {
                    console.log("Error is 403 forbiden in head request. File size is previously known..");
                    self.headers = {'content-length': self._options.file_size, 'content-type': self._options.content_type};
                    if(self._options.accept_ranges) {
                        console.log("Error is 403 forbiden in head request. Server is know to support ranges..");
                        self.headers['accept-ranges'] = 'bytes';
                    }
                    self.file_size = Number(self.headers['content-length']);
                    console.log("filesize %d", self.file_size)
                    if (self._options.destFile) {
                        console.log("_init_file_stream")
                        self._init_file_stream.call(self, function (error) {
                            if (error) { next(error); return; }
                            console.log("_init_http")
                    ...
                    console.log("Error is 403 forbiden in head request. Trying with get request..");
                    request.get(self._url, _.clone(self._options, true), function (error, response) {
                        console.log("Got reply from head request..");
            ...
            next(new Error('http status code ' + response.statusCode)); return;
        }
...
FastDownload.prototype._init_http = function (cb) {
    ...
    if (accept_ranges) {
        console.log("_init_fast_http");
        self._init_fast_http(cb);
    } else {
        console.log("_init_normal_http");
...
function Chunk(dl, number) {
    ...
    self._req.on('data', function (data) {
        ...
        if (self.position == self.size) {
            ...
            self.emit('progress', dl.downloaded);
        }
    });
}
